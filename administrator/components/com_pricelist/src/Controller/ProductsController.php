...
class ProductsController extends AdminController
{
    ...
    public function featured()
    {
        ...
        foreach ($ids as $i => $id)
        {
            ...
            if (!$this->app->getIdentity()->authorise('core.edit.state', 'com_pricelist.category.' . (int)$item->catid))
            {
        ...
        if (empty($ids))
        {
            $this->app->enqueueMessage(Text::_('COM_PRICELIST_NO_ITEM_SELECTED'), 'warning');
        }
        else
        {
            // Publish the items.
            ...
            if ($value == 1)
            {
                $message = Text::plural('COM_PRICELIST_N_ITEMS_FEATURED', count($ids));
            }
            else
            {
                $message = Text::plural('COM_PRICELIST_N_ITEMS_UNFEATURED', count($ids));
            }
        }

        $this->setRedirect('index.php?option=com_pricelist&view=products', $message);
    }
